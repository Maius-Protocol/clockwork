...
use miclockwork_thread_program::state::{SerializableInstruction, Thread, ThreadSettings, Trigger, VersionedThread};
use miclockwork_utils::CrateInfo;
...
pub fn crate_info(client: &Client) -> Result<(), CliError> {
    let ix = Instruction {
        ...
        accounts: miclockwork_thread_program::accounts::GetCrateInfo {
            system_program: system_program::ID,
        }.to_account_metas(Some(false)),
        data: miclockwork_thread_program::instruction::GetCrateInfo {}.data(),
    };
...
pub fn delete(client: &Client, id: String) -> Result<(), CliError> {
    ...
    let ix = Instruction {
        program_id: miclockwork_thread_program::ID,
        accounts: miclockwork_thread_program::accounts::ThreadDelete {
            authority: client.payer_pubkey(),
            close_to: client.payer_pubkey(),
            thread: thread_pubkey,
        }.to_account_metas(Some(false)),
        data: miclockwork_thread_program::instruction::ThreadDelete {}.data(),
    };
    client.send_and_confirm(&[ix], &[client.payer()]).unwrap();
    Ok(())
}
...
pub fn pause(client: &Client, id: String) -> Result<(), CliError> {
    ...
    let ix = Instruction {
        program_id: miclockwork_thread_program::ID,
        accounts: miclockwork_thread_program::accounts::ThreadPause {
            authority: client.payer_pubkey(),
            thread: thread_pubkey,
        }.to_account_metas(Some(false)),
        data: miclockwork_thread_program::instruction::ThreadPause {}.data(),
    };
    client.send_and_confirm(&[ix], &[client.payer()]).unwrap();
    get(client, thread_pubkey)?;
    Ok(())
...
pub fn resume(client: &Client, id: String) -> Result<(), CliError> {
    ...
    let ix = Instruction {
        program_id: miclockwork_thread_program::ID,
        accounts: miclockwork_thread_program::accounts::ThreadResume {
            authority: client.payer_pubkey(),
            thread: thread_pubkey
        }.to_account_metas(Some(false)),
        data: miclockwork_thread_program::instruction::ThreadResume {}.data(),
    };
    client.send_and_confirm(&[ix], &[client.payer()]).unwrap();
    get(client, thread_pubkey)?;
    Ok(())
...
pub fn reset(client: &Client, id: String) -> Result<(), CliError> {
    ...
    let ix = Instruction {
        program_id: miclockwork_thread_program::ID,
        accounts: miclockwork_thread_program::accounts::ThreadReset {
            authority: client.payer_pubkey(),
            thread: thread_pubkey
        }.to_account_metas(Some(false)),
        data: miclockwork_thread_program::instruction::ThreadReset {}.data(),
    };
    client.send_and_confirm(&[ix], &[client.payer()]).unwrap();
    get(client, thread_pubkey)?;
    Ok(())
...
pub fn update(
    ...
    let ix = Instruction {
        program_id: miclockwork_thread_program::ID,
        accounts: miclockwork_thread_program::accounts::ThreadUpdate {
            authority: client.payer_pubkey(),
            system_program: system_program::ID,
            thread: thread_pubkey
        }.to_account_metas(Some(false)),
        data: miclockwork_thread_program::instruction::ThreadUpdate { settings }.data(),
    };
    client.send_and_confirm(&[ix], &[client.payer()]).unwrap();
    get(client, thread_pubkey)?;
    Ok(())
