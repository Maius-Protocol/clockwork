...
use miclockwork_network_program::state::{
...
pub fn create(client: &Client, signatory: Keypair, silent: bool) -> Result<(), CliError> {
    ...
    let ix = Instruction {
        ...
        accounts: miclockwork_network_program::accounts::WorkerCreate {
        ...
        data: miclockwork_network_program::instruction::WorkerCreate {}.data(),
    };
    client
...
pub fn update(client: &Client, id: u64, signatory: Option<Keypair>) -> Result<(), CliError> {
    ...
    let ix = Instruction {
        program_id: miclockwork_network_program::ID,
        accounts: miclockwork_network_program::accounts::WorkerUpdate {
            authority: client.payer_pubkey(),
            system_program: system_program::ID,
            worker: worker_pubkey,
        }.to_account_metas(Some(false)),
        data: miclockwork_network_program::instruction::WorkerUpdate { settings }.data(),
    };
