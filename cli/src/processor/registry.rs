...
use miclockwork_network_program::state::{Config, Registry, Snapshot};

use crate::{client::Client, errors::CliError};

pub fn get(client: &Client) -> Result<(), CliError> {
    let registry_pubkey = miclockwork_network_program::state::Registry::pubkey();
    let registry = client
...
pub fn unlock(client: &Client) -> Result<(), CliError> {
    let ix = Instruction {
        ...
        accounts: miclockwork_network_program::accounts::RegistryUnlock {
            admin: client.payer_pubkey(),
            config: Config::pubkey(),
            registry: Registry::pubkey()
        }.to_account_metas(Some(false)),
        data: miclockwork_network_program::instruction::RegistryUnlock {}.data(),
    };
