pub use miclockwork_thread_program::errors;
pub use miclockwork_thread_program::program::ThreadProgram;
pub use miclockwork_thread_program::ID;

pub mod state {
    pub use miclockwork_thread_program::state::{
...
pub mod utils {
    pub use miclockwork_thread_program::state::PAYER_PUBKEY;
    pub use miclockwork_thread_program::state::Equality;
}

pub mod cpi {
    use anchor_lang::prelude::{CpiContext, Result};

    pub use miclockwork_thread_program::cpi::accounts::{
    ...
    pub fn thread_create<'info>(
        ...
        miclockwork_thread_program::cpi::thread_create(ctx, amount, id, instructions, trigger)
    }

    pub fn thread_delete<'info>(
        ctx: CpiContext<'_, '_, '_, 'info, ThreadDelete<'info>>,
    ) -> Result<()> {
        miclockwork_thread_program::cpi::thread_delete(ctx)
    }

    pub fn thread_pause<'info>(
        ctx: CpiContext<'_, '_, '_, 'info, ThreadPause<'info>>,
    ) -> Result<()> {
        miclockwork_thread_program::cpi::thread_pause(ctx)
    }

    pub fn thread_resume<'info>(
        ctx: CpiContext<'_, '_, '_, 'info, ThreadResume<'info>>,
    ) -> Result<()> {
        miclockwork_thread_program::cpi::thread_resume(ctx)
    }

    pub fn thread_reset<'info>(
        ctx: CpiContext<'_, '_, '_, 'info, ThreadReset<'info>>,
    ) -> Result<()> {
        miclockwork_thread_program::cpi::thread_reset(ctx)
    }
    ...
    pub fn thread_update<'info>(
        ...
        miclockwork_thread_program::cpi::thread_update(ctx, settings)
    }
    ...
    pub fn thread_withdraw<'info>(
        ...
        miclockwork_thread_program::cpi::thread_withdraw(ctx, amount)
    }
}
