...
use miclockwork_thread_program::state::{Thread as ThreadV2, VersionedThread};
use miclockwork_thread_program_v1::state::Thread as ThreadV1;
...
impl TryFrom<&mut ReplicaAccountInfo<'_>> for AccountUpdateEvent {
    ...
    fn try_from(account_info: &mut ReplicaAccountInfo) -> Result<Self, Self::Error> {
        ...
        if owner_pubkey.eq(&miclockwork_thread_program_v1::ID) && account_info.data.len() > 8 {
        ...
        if owner_pubkey.eq(&miclockwork_thread_program::ID) && account_info.data.len() > 8 {
            let d = &account_info.data[..8];
